...
const Topbar = ({
  ...
  const title = pathname.includes("clients")
    ? "Clients"
    : pathname.includes("employees")
    ? "Employees"
    : "";

  const handleToggleFilters = () => setOpenFilters((prev) => !prev);

  const handleSearch = (e) => {
    dispatch(searchUserReducer(e.target.value));
  };
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex justify-between items-center mb-5">
        <h1 className="text-primary-blue text-2xl capitalize font-light">{title}</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Tooltip title="Filter" arrow>
            ...
            >
              <FiFilter className="text-[22px]" />
            </div>
          </Tooltip>

          {showAddClient && (
            <Tooltip title={`Add New ${title.slice(0, -1)}`} arrow>
